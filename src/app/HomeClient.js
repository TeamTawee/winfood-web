...
import { ArrowRight, ChevronRight, ChevronLeft, Image as ImageIcon, PackageOpen } from "lucide-react";
...
function MouseParallax({ children, limitY = false }) {
    ...
    function handleMouseMove(event) {
        ...
        x.set(xVal); y.set(yVal);
    }
    
    return <motion.div onMouseMove={handleMouseMove} style={{ x: smoothX, y: smoothY }} className="w-full h-full flex items-center justify-center">{children}</motion.div>;
}
...
// 🟢 Component หลักที่รับข้อมูลที่ดึงเสร็จแล้วมาจาก Server
export default function HomeClient({ initialProducts, initialBanners }) {
        ...
        <div className="max-w-7xl mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center h-full relative z-10">
            ...
            <div className="relative h-100 lg:h-162.5 w-full flex items-end justify-center mt-8 lg:mt-0">
                <MouseParallax limitY={true}> 
                    <motion.div initial={{ opacity: 0, y: 40, scale: 1 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 1, delay: 0.2 }} className="relative w-full h-full flex items-end justify-center">
                        <Image src="/images/hero main.png" alt="Hero Product" fill className="object-contain object-bottom pointer-events-none drop-shadow-2xl translate-y-0" priority />
                    </motion.div>
                </MouseParallax>
            </div>
        </div>
                        ...
                        {banners[currentBanner].image ? (
                            banners[currentBanner].link && banners[currentBanner].link.trim() !== "" && banners[currentBanner].link.trim() !== "#" ? (
                                <Link href={banners[currentBanner].link} target="_blank" rel="noopener noreferrer" className="block w-full h-full cursor-pointer relative z-20">
                                    <Image src={banners[currentBanner].image} alt="Banner" fill className="object-cover" priority={currentBanner === 0} />
                                </Link>
                            ) : (
                            ...
                            <div className="w-full h-full bg-slate-200 flex items-center justify-center relative z-20"><ImageIcon className="text-slate-400" size={64} /></div>
                        )}
                    </motion.div>
                </AnimatePresence>
            ) : (
      ...
      <section className="py-24 px-6 bg-white relative overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-16 relative z-10">
            <div className="flex-1 space-y-6 text-center md:text-left">
                ...
                <h2 className="text-2xl md:text-3xl font-bold text-slate-900 tracking-tight leading-tight whitespace-pre-line text-balance">
            ...
            <div className="flex-1 relative w-full h-64 md:h-96">
                 <ScrollParallax speed={0.2}>
                    <div className="relative w-full h-full rounded-2xl overflow-hidden shadow-2xl">
                        <Image src="https://firebasestorage.googleapis.com/v0/b/winfood-web.firebasestorage.app/o/banners%2Fallprowf%20Large.jpeg?alt=media&token=7a2dc607-d7c3-4795-8dbe-c5c630f19b2f" alt="About Preview" fill className="object-cover hover:scale-105 transition-transform duration-1000" />
                    </div>
                 </ScrollParallax>
